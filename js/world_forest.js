...
import { mulberry32, hashStringToSeed, pick } from "./utils.js";
...
export function buildForestWorld(scene, seedStr="forest-01"){
  ...
  const ground = new THREE.Mesh(new THREE.PlaneGeometry(140, 140, 1, 1), groundMat);
  ground.rotation.x = -Math.PI/2;
  ground.receiveShadow = true;
  group.add(ground);

  // trees
  const trunkMat = new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 1.0 });
  const leafMat = new THREE.MeshStandardMaterial({ color: 0x0d1712, roughness: 1.0, metalness: 0.0 });
  ...
  // interactions: a “portal” to city
  const portal = new THREE.Mesh(
    new THREE.TorusGeometry(1.2, 0.18, 10, 24),
    new THREE.MeshStandardMaterial({ color: 0xd6dbe6, roughness: 0.25, metalness: 0.2, emissive: 0x223344, emissiveIntensity: 0.8 })
  );

...
export class AudioEngine {
  ...
  async start(){
    ...
    // Noise bed (field-recording-like)
    this.noiseGain = this.ctx.createGain();
    this.noiseGain.gain.value = 0.0;
    this.noiseGain.connect(this.master);

    this.noiseSrc = this._makeBrownNoise();
    this.noiseSrc.connect(this.noiseGain);
    this.noiseSrc.start();

    // Drone (minor-ish cluster)
    this.droneGain = this.ctx.createGain();
    this.droneGain.gain.value = 0.0;
    this.droneGain.connect(this.master);

    const base = 55; // A1
    const ratios = [1, 6/5, 3/2, 9/5]; // dark-ish
    for (let i=0;i<4;i++){
  ...
  tick(dt){
    ...
    // target levels scale with dread
  ...
  _makeBrownNoise(){
    ...
    // return node as if source output
    lp._source = src;
    return lp;
  }
}
